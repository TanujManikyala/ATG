...
$query = mysqli_query(mysqli_connect("localhost","id20469554_root","a@/jC1T-JrdVL5M!","id20469554_ttms"), "SELECT * FROM subjects ");
...
while ($row = mysqli_fetch_assoc($query)) {
    ...
    if (isset($temp->subjectteacher)) {
        $teacheralias_query = mysqli_query(mysqli_connect("localhost","id20469554_root","a@/jC1T-JrdVL5M!","id20469554_ttms"),
...
$query = mysqli_query(mysqli_connect("localhost","id20469554_root","a@/jC1T-JrdVL5M!","id20469554_ttms"), "SELECT * FROM teachers ");
...
$username = "id20469554_root";
$password = "a@/jC1T-JrdVL5M!";
...
// Retrieving the previously stored value from the database
$sql = "SELECT semester FROM temp ORDER BY id DESC LIMIT 1";
...
// Displaying the retrieved value on the screen
...
for ($I = 0; $I < $subjects_count * 4; $I++) {
    ...
    $year = ($sem + 1) / 2;
    $classroom_query = mysqli_query(mysqli_connect("localhost","id20469554_root","a@/jC1T-JrdVL5M!","id20469554_ttms"),
...
/**********************check for empty slots in teacher's timetable*******************************/
    ...
    for ($k = 0; $k < 6; $k++) {
        for ($j = 0; $j < 5; $j++) {
            ...
            if (isset($teachers[$i]->days[$final][$k][$j])) {
            } else {
                $teachers[$i]->days[$final][$k][$j] = "-";
                $teachers[$i]->classroom_names[$final][$k][$j] = "-";
...
for ($i = 0; $i < $teachers_count; $i++) {
    for ($k = 0; $k < 6; $k++) {
        ...
        if (isset($teachers[$i]->days[$final][$k][5])) {}
        else {
            $teachers[$i]->days[$final][$k][5] = "-";
        }
    }
}
...
period6='" . $subjectslots[$i][$j][5] . "<br>" . $aliasslots[$i][$j][5][0] . ", " . $aliasslots[$i][$j][5][1] . ", " . $aliasslots[$i][$j][5][2] . "'
 WHERE day='" . $days[$j] . "' ";
        $q = mysqli_query(mysqli_connect("localhost","id20469554_root","a@/jC1T-JrdVL5M!","id20469554_ttms"), $query);
    }
...
for ($i = 0; $i < $teachers_count; $i++) {
    ...
    for ($j = 0; $j < 6; $j++) {
        $query = "UPDATE" . $database_name . " SET  period1= '" . $teachers[$i]->days[$final][$j][0] . "<br>" . $teachers[$i]->classroom_names[$final][$j][0] . "',
period2='" . $teachers[$i]->days[$final][$j][1] . "<br>" . $teachers[$i]->classroom_names[$final][$j][1] . "', 
period3='" . $teachers[$i]->days[$final][$j][2] . "<br>" . $teachers[$i]->classroom_names[$final][$j][2] . "',
period4='" . $teachers[$i]->days[$final][$j][3] . "<br>" . $teachers[$i]->classroom_names[$final][$j][3] . "', 
period5='" . $teachers[$i]->days[$final][$j][4] . "<br>" . $teachers[$i]->classroom_names[$final][$j][4] . "',
period6='" . $teachers[$i]->days[$final][$j][5] . "'
 WHERE day='" . $days[$j] . "' ";
